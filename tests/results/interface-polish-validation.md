...
用户应运行更新后的完整目录包；其他电脑已安装的副本和会议历史不会因本地包同步而自动更新。运行中更改操作系统语言通知仍未接入；应用重启读取系统首选语言。真实模型／转写效果不在本轮验收范围。代码与包已同步，Windows运行待验；随后用户授权交付，源码91962b9已推送origin/main并回读确认，测试及包指纹保持一致。
