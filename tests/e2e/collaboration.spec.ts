...
test('host publishes a poll to independent participants; participant cannot read the host snapshot', async () => {
  ...
  try {
    ...
    await expect
      ...
      .toBe(true);
    const component = (await app.windows()).find((p) => /[?&]role=component(?:&|$)/.test(p.url()))!;
    ...
    await component.getByRole('button', { name: '审核并分发给3人', exact: true }).click();
...
test('assignment objections produce a floating conflict, resolution drafts and explicit three-person confirmation', async () => {
  ...
  try {
    ...
    await expect
      .poll(
        async () =>
          (await app.windows()).filter((p) => /[?&]role=component(?:&|$)/.test(p.url())).length,
      )
      .toBe(1);
    const assignment = (await app.windows()).find((p) =>
      /[?&]role=component(?:&|$)/.test(p.url()),
    )!;
    ...
    await expect(assignment.getByRole('button', { name: '审核并分发给3人' })).toBeDisabled();
    await assignment.getByRole('button', { name: '保存草稿', exact: true }).click();
    await assignment.getByRole('button', { name: '审核并分发给3人' }).click();
    await host.getByRole('button', { name: '打开 参与者A' }).click();
    await expect
      .poll(
        async () =>
    ...
    const conflict = (await app.windows())
      .filter((p) => /[?&]role=component(?:&|$)/.test(p.url()))
      .find((p) => p !== assignment)!;
    ...
    await conflict.getByText('调整受众与共享依据（可选）', { exact: true }).click();
    await conflict.getByText('公开依据摘录', { exact: true }).click();
    await conflict.locator('.evidence-choice input').check();
    await conflict.getByRole('button', { name: '审核并分发给3人' }).click();
    ...
    await confirmation.getByRole('button', { name: '审核并分发给3人' }).click();
    ...
    await app.evaluate(({ BrowserWindow }) =>
      BrowserWindow.getAllWindows()
        .filter((w) => /[?&]role=component(?:&|$)/.test(w.webContents.getURL()))
