...
export function IntentPanel({
  ...
  const perform = async (type: string, payload: Record<string, unknown>) => {
    ...
    try {
      ...
      if (type === 'collaborationPromote' && typeof result === 'string')
        await api('openComponent', { meetingId: meeting.id, componentId: result });
      setError('');
    } catch (e) {
    ...
    <section className="intent-panel" data-testid="intent-panel">
      ...
      {state?.enabled && (
        <>
          <p>
            {zh
              ? '实验性自动草稿，仅发起者可见。核对后可转为本地协作组件，继续预览、选择参与者并发放。'
              : 'Experimental private drafts. Review and transfer to a local collaboration component, then preview and explicitly publish to participants.'}
          </p>
                ...
                >
                  ...
                  <div className="intent-actions">
                    {c &&
                      ...
                      ['prepare', 'update', 'propose_resolution'].includes(
                        d.candidate.operation,
                      ) && (
                        <button
                          disabled={ended || busy || d.needsReview}
                          onClick={() => void perform('collaborationPromote', base)}
                        >
                          {zh
                            ? d.promotedComponentId
                              ? '打开／更新协作草稿'
                              : '转为本地协作组件'
                            : d.promotedComponentId
                              ? 'Open/update collaboration draft'
                              : 'Create local collaboration component'}
                        </button>
                      )}
