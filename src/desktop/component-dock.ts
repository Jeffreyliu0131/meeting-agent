...
export class ComponentDock {
  ...
  view(): ComponentDockView {
    ...
    const cards = (s?.components ?? [])
      .filter((c) => {
        ...
        return content.kind === 'poll'
          ? !!content.payload.question.trim() && content.payload.options.some((o) => o.label.trim())
          : content.kind === 'assignment'
            ? content.payload.items.some((i) => i.title.trim())
            : content.kind === 'conflict'
              ? content.payload.sides.some((i) => i.title.trim())
              : !!content.payload.statement.trim();
      })
      .map((c) => ({
        id: c.id,
        revision: c.draftRevision,
        content: c.revisions.at(-1)!.content,
        preparing:
          c.draftState === 'collecting' ||
          !!s?.jobs.some(
            (j) => j.componentId === c.id && ['pending', 'running'].includes(j.status),
          ),
      }));
    ...
    return {
      ...
      locale: state?.preferences.uiLocale ?? 'zh-CN',
      cards,
  ...
  sync() {
    ...
    const height = Math.min(
      expanded ? 760 : 74 + Math.min(view.cards.length, 3) * 146,
      work.height - 48,
    );
    const bounds = {
      x: work.x + work.width - width - 16,
      y: work.y + Math.min(80, Math.max(16, work.height - height - 16)),
      width,
      height,
    };
    ...
    if (
      Object.keys(bounds).some(
        (k) => old[k as keyof typeof old] !== bounds[k as keyof typeof bounds],
      )
    )
  ...
  async handle(method: string, args: any) {
    ...
    if (method === 'componentDockInspect') {
      ...
      if (!this.pinned || args?.pin === true) {
        this.selectedId = args.componentId;
        this.pinned = args.pin === true;
        this.sync();
    ...
    if (method === 'collaborationSnapshot') {
      const result = await this.ports.request(method, {
        meetingId: view.meetingId,
        actorId: host.id,
      });
      return {
        ...result,
        assignmentDirectory: result.components.flatMap((c: any) =>
    ...
    if (
      ...
      args?.meetingId === view.meetingId &&
      args?.payload?.componentId === view.selectedId
    ) {
      this.pinned = true;
