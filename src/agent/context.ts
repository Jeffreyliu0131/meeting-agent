...
export function buildContextBatch(m: Meeting, maxBytes = 28000): ContextBatch {
  const all = latestSegments(m),
    pending = pendingSegments(m).sort((a, b) => sourceVersion(a) - sourceVersion(b));
  ...
  const references = [...objects.flatMap((o) => o.sources), ...relations.flatMap((r) => r.sources)];
  const recalled = all
    ...
    .map((x) => x.s);
  const extras = all
    .filter((s) => references.some((r) => r.id === s.id))
  ...
  while (size() > maxBytes && projected.artifacts.length) projected.artifacts.pop();
  while (size() > maxBytes && projected.objects.length) {
  ...
  if (size() > maxBytes) throw new Error('CONTEXT_SOURCE_TOO_LARGE');
...
export function contextPayload(m: Meeting) {
  ...
  return {
    ...
    decisions: m.decisions.slice(-3).map((d) => ({
      scope: d.scope,
      basis: d.basis,
