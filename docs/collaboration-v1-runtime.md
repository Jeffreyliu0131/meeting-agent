...
日期：2026-09-12，YH `a91dcce`后的审核流程修正工作树；本轮未提交；自动缩略区修正后已重启Windows日常进程，见[验证](../tests/results/component-dock-validation.md)。早期PR交付不代表本次修正已交付。本文说明实际实现；[设计](collaboration-v1-design.md)保留完整目标，[验证](../tests/results/collaboration-v1-validation.md)说明证据范围。

## 使用路径

正常音频会议在创建时自动建立本地发起者＋参与者A／B／C四个模拟身份；恢复中的活动音频会议补齐此配置。此初始化不会启动录音。手工／回放开发输入保留显式启用。身份来自可信窗口绑定，音源或发言中的姓名没有投票权限。

Agent静默分析参会者之间的自然讨论，推断现实协作需求，生成完整的投票、分工、冲突或决定确认，在独立置顶悬浮窗提供只读审核。已讨论的选项或结论使用回顾准备；仅等待未来补充组件内容时处于收集态，等待投票／接受／同意不属于准备收集。屏幕右侧固定区自动显示带真实内容的缩略卡；已有内容的收集态也会出现并随来源更新，空白表不冒出。悬停向左展开完整审核，移出收起；点击固定，host审核后一键分发。更新保持组件与选项ID，不要求重新打开。主工作区索引及悬浮球数量为辅助入口。自动显示不抢焦点；收起不取消或停止会议。
