...
PR #4已合入main，在上述主线上整合静默识别、完整审核、一键分发和自动组件缩略区；本轮合并与双端验证见[交接](sessions/2026-09-12-pr4-integration.md)。

Windows旧运行包的自然音频实测出现积压／超时；不能以文本增量测试代替完整音频通过，见[本轮实测](sessions/2026-09-12-full-audio-simulation.md)。

## 使用路径

正常音频会议在创建时自动建立本地发起者＋参与者A／B／C四个模拟身份；恢复中的活动音频会议补齐此配置。此初始化不会启动录音。手工／回放开发输入保留显式启用。身份来自可信窗口绑定，音源或发言中的姓名没有投票权限。

Agent静默分析参会者之间的自然讨论，推断现实协作需求，生成完整的投票、分工、冲突或决定确认，在独立置顶悬浮窗提供只读审核。已讨论的选项或结论使用回顾准备；仅等待未来补充组件内容时处于收集态，等待投票／接受／同意不属于准备收集。屏幕右侧固定区自动显示带真实内容的缩略卡；已有内容的收集态也会出现并随来源更新，空白表不冒出。悬停向左展开完整审核，移出收起；点击固定，host审核后一键分发。更新保持组件与选项ID，不要求重新打开。主工作区索引及悬浮球数量为辅助入口。自动显示不抢焦点；收起不取消或停止会议。缩略区标题与状态跟随界面中英文。
...
运行时契约在[collaboration.ts](../src/contracts/collaboration.ts)。四类payload使用严格判别联合；模型提案不能写actor、计数或公开状态。草稿revision不可变追加，仅受众建议改变也生成新审核版本；round冻结内容、受众、共享依据；response带公开版本、本人subject及本人responseVersion。当前草稿与公开轮可以并存。
