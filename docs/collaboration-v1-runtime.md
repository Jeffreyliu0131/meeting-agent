...
PR #4在上述主线上整合静默识别、完整审核、一键分发和自动组件缩略区；本轮合并与双端验证见[交接](sessions/2026-09-12-pr4-integration.md)。

## 使用路径

正常音频会议在创建时自动建立本地发起者＋参与者A／B／C四个模拟身份；恢复中的活动音频会议补齐此配置。此初始化不会启动录音。手工／回放开发输入保留显式启用。身份来自可信窗口绑定，音源或发言中的姓名没有投票权限。

Agent静默分析参会者之间的自然讨论，推断现实协作需求，生成完整的投票、分工、冲突或决定确认，在独立置顶悬浮窗提供只读审核。已讨论的选项或结论使用回顾准备；仅等待未来补充组件内容时处于收集态，等待投票／接受／同意不属于准备收集。主工作区保留索引；屏幕右侧固定缩略区自动展示准备好的未发放组件，有实际内容的收集草稿标为正在补充。悬停展开完整预览，点击固定审核，一键分发；悬浮球仍保留数量与打开入口。缩略区标题与状态跟随界面中英文。Agent准备完成不会自动抢焦点；关闭悬浮窗不会取消组件、提交回应或停止会议。
