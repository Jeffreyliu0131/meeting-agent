...
用户选定本任务第一张正文／原话边注图，要求右栏稍窄，随后授权实现。采用[专项规范](meeting-editorial.md)及[细化参考](assets/meeting-editorial-reference.png)。这修订此前8图中会中／来源的比例和正文容器，不替代首页、设置、桌面入口方向；Agent结构不固定。视觉稿不能证明真实转写、模型和双端体验，实际证据见[本轮交接](../sessions/2026-09-12-meeting-editorial.md)。
...
- 当前视觉与页面规范：[style.md](style.md)，运行时标识 `collaborative-light-v2`；接入与后续飞书布局细化见[前端改造](../sessions/2026-09-11-frontend-refresh.md)。
...
- [前端规范](../frontend-spec.md)与[会议入口](../meeting-entry-spec.md)继续约束行为；style.md 已列出与生成端原文的校准。[tokens.json](tokens.json)和源码已由后续前端任务统一迁移，程序与视觉验证单独记录。

## 替代与验证边界

本决定替代下方历史记录中“旧第二张是当前视觉唯一基准”的要求。保留浅色、低打扰和工作内容可读性，但不继续强制旧图构图、字号和配色。旧图片不删除、不移动，以免破坏历史依据。

新视觉参数是实施起点。局部静态色对计算见 style.md，不能据此认定整套 UI 已通过对比度或无障碍验收。首次初始化、代表性交互、尺寸和键盘已做本地程序／合成视觉验证；真实场景、双端与完整无障碍效果仍须独立验收。本页原始决定属于文档整理；后续前端代码变化见其独立记录，不将概念稿认可当作实现验收。
