...
发布／记录命令由服务生成`evaluationFrontier`，保存点击时已接收final来源refs并等待该范围语义处理。处理产生依赖变化时命令返回需重新预览的新版本，不能替host批准变动后的内容。前沿未处理完不开始业务事务；waiting状态持久化且可以取消。record_decision不能在相关来源缺口或分析欠账下成功。
