...
四类私有意图由既有理解图同次输出；正式协作使用独立 C／R 图和可信发布服务。启用私有自动准备后，同次理解不再重复调度另一套自动组件；用户明确转交后仍须另行发放。两者的状态、保护与恢复边界见[意图说明](collaboration-intents.md)。

本文从 `9b6886d` 的工作流基线续修，包含已合入 `55267d6` 的协作／集合与后续实时表达改动。早期[实施任务书](agent-workflow-implementation.md)、[验收场景](agent-workflow-acceptance.md)和[独立验收](../tests/results/agent-workflow-acceptance-review.md)保留原版本范围；最新结果从[状态](status.md)进入。本文描述源码行为；真实语义、音源、Windows 实机结果不能从代码或合成测试推导。
...
供应商调用池总上限4；转写用途最多2个并发任务以覆盖麦克风和电脑声音，其余每种用途最多1个。所有非理解调用合计最多占3个槽，给会议理解留1个槽；新增集合用途也受同一总上限约束。个人4次模型、6次工具为持久上限；当前图每次模型最多提出1个工具请求，因此4次模型实际最多完成3次工具再返回最终结果。实时工具最多1次。表达最多2次生成；直接产物预览失败最多1次修复。已有小时调用／token预算继续执行。账目在调用前落库，崩溃不退款；排队后取消可能保守计为已用额度，不会超额补发。
...
生产理解上下文上限取配置与完整请求60,000字节预算的较小值：理解路径扣除实际 SYSTEM、额外指令预留1,024字节、当前 JSON Schema 与512字节余量。schema 已复用共享定义，旧约19KB测量不再代表当前容量；本轮零请求计算得到默认理解24,000字节、集合32,000字节。补证据后的完整请求仍再次检查，超限明确失败而不截断原话。集合可通过 `MEETING_COLLECTION_CONTEXT_BYTES` 单独配置（12,000–48,000），两项容量都不等于无限历史覆盖。
...
用户从会议列表创建主题文件夹，显式选择1–8场会议；集合定义、成员、输出语言和报告历史存入同一 SQLite 聚合状态。删除文件夹只移除集合及其报告，成员会议保留。`saveState()` 默认携带现有集合，其他协作／音频／工作流写入不会因遗漏参数清空它们。

`collectionDecisions()` 合并投影旧产物型会议决定与正式协作的已记录决定；个人采纳不进入该列表。确定性汇总同时列出未决项、分歧和覆盖省略量。模型只收到别名后的内容；真实 ID 映射留在主机。决定别名是可引用的业务记录，不成为原话或语义对象；当前界面可返回所属会议，尚无精确决定详情跳转。

手动生成由 `drainCollectionReport()` 调用独立 `ModelPort.synthesize()`，不调用会议理解图、不写成员对象／关系、不开放 evidence 工具。正常一次模型调用，返回后的报告校验失败最多再修复一次；网络失败不会在该循环里自动重试。最多12块、无公式，复用既有内容和引用校验／渲染器；这些检查不能保证模型正确呈现所有分歧或数字。

生成起点冻结集合定义、成员和会议／语言版本。期间任一版本变化返回 `COLLECTION_CHANGED`，保留旧报告；没有可引用原话或确认记录返回 `COLLECTION_EMPTY`。成功时追加不可变快照、别名映射、实际引用、水位及省略量。之后的变化只显示过期提示，不自动重生。

集合使用独立小时账目和共享调用池，不增加总并发。重启保留历史报告和已用账目，清除运行中状态，不自动续跑报告；用户可重新发起。当前 UI 只展示最新报告，尚无历史版本选择器和独立报告导出。真实语义与 Windows 运行验收仍见[整合记录](sessions/2026-09-12-branch-integration.md)的未测项。

## 实际模型响应与可见草稿

模型请求使用 Chat Completions 的 `stream: true` 和 `stream_options.include_usage`；兼容端点若返回完整 JSON 也可解析，但不会出现中途草稿。仅把响应中的 focus／question／summary 提取为最多240字符的可见暂定文字，不显示推理或未验证的 HTML／SVG。最终响应完成并通过本地 schema／业务校验后才提交；服务清理瞬态草稿。HTTP失败或流中断不会静默重发非流式请求。此为协议与展示机制，不证明真实供应商质量或端到端时延，见[实时表达验证](../tests/results/demo-live-visuals-validation.md)。
