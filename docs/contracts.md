...
本文件中的抽象契约／后续设计不能全部视为已实现。接手时先按下表和[ADR-004](adr/004-live-agent-pipeline.md)核对源码：

| 设计描述 | 当前实际实现／剩余边界 |
|---|---|
| SemanticDelta、ExpressionPlan、生成工作流 | 实际统一Proposal返回语义及互斥artifact／patch／plan；复杂plan独立生成。格式校验仍覆盖整个Proposal，格式错误不等于可以保留其中任意部分 |
| 服务分配ID、批内临时ID映射 | 当前语义对象ID由模型提出、服务校验并沿用；尚无完整的临时ID分配映射协议 |
| jobs租约、完整追踪与p95 | 当前为持久化表达任务数组、来源水位、调用记录及内存调度；尚无完整租约调度系统或真实p95证据 |
| 多表／依赖索引存储 | 当前SQLite保存事件聚合快照和幂等命令；大历史分页与规范化多表仍为后续演进 |
