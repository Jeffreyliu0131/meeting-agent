...
- 在完整设置中选择Interface language立即以preferencesPatch独立保存并更新可信外壳。其他设置也改为字段级自动保存，写入队列保留后续用户意图，不把整份过期草稿覆盖到新状态。失败回到已保存值并就地提示／重试；不停止会议、不触发全场模型重跑，也不改变当前输出语言。详见[即时设置](meeting-entry-spec.md#11-设置即时保存2026-09-12)。
